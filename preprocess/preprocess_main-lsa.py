...
def preparedata(data, params):
    ...
    if params["RESTRICTION"] == True:
        ...
        traincond1 = df_train["doc len"] >= 972 #485, 972
        traincond2 = df_train["doc len"] <= 1024 #512, 1024
        traincond3 = df_train["sum len"] <= 36

        testcond1 = df_test["doc len"] >= 972 #485, 972
        testcond2 = df_test["doc len"] <= 1024 #512, 1024
...
def process(loader, params, mode):
    ...
    for _, data in enumerate(loader, 0):
        ...
        if not os.path.exists(f"""preprocessed_text/{params["METHOD"]}_1024/quantity_{params["SHORTENING QUANTITY"]}/{mode}"""):
            os.makedirs(f"""preprocessed_text/{params["METHOD"]}_1024/quantity_{params["SHORTENING QUANTITY"]}/{mode}""")
        final_df.to_csv(f"""preprocessed_text/{params["METHOD"]}_1024/quantity_{params["SHORTENING QUANTITY"]}/{mode}/{params["METHOD"]}_quantity{params["SHORTENING QUANTITY"]}_step{_}.csv""")

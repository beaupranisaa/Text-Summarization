...
def get_dataset(dataset, mode, config):      
    ...
    index = np.random.permutation(index)[3456:]
...
def process(loader, config, mode):
    ...
    path = f"""preprocessed_text/{config['orig_source_length']}_{config['method']}/quantity_{config['max_source_length']}/{mode}/"""
...
config = dict(
    ...
    max_source_length = 373, # 512, 373, 323, 273
